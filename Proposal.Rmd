...
2. User has the ability to scope to the interested date range.This control will
   alter all 3 charts found within the pane.              
...
'venueId' based on the total revenue generated in descending order. At a glance,
users are able to identify the performing and struggling businesses.Users can 
hover over the bar charts to obtain the total revenue amount for each businesses.
...
Users have the ability to zoom in and out of the plot as well as hover onto the
circles to identify the 'venue ID' and the total sales collected.
...
2.  A violin-and-boxplot that shows the statistical distribution of
...
### 5. Distribution of Daily Sales by Weekday

![Average Spending of Customers Chart](images/Q1Deep1.png)

User can compare the daily sales amount based on the weekdays.
...
non-peak periods of the selected businesses.User can also hover on the heatmap 
to view the average number of vistors at a selected hour.
...
3.  A box-plot that shows the distribution of average hourly wage of all
    employers.
4.  A data table that displays the venueId, number of jobs, average hourly wage 
    and total hourly wage details.

### 1. Control

![](images/Q1ControlWages.png)

1. User can filter based on the 'Education Level'. This applies to all 3 charts 
   found within the pane.

### 2. Packed Bar Chart of Average Hourly Wage

![Average Hourly Wage Packed Bar Chart](images/Q1Wages1.png)

The top 10 employers that offers highest average hourly wage are highlighted in 
blue. User can hover onto the bar chart to view the employer ID and the average 
hourly wage offered by the respective employer.
...
User can hover the box-plot and data points to view the average hourly wage 
offered.User can also select the data points to reveal more details on the
selected points.
...
When a user select a sets of data points from the 
